...
admissions_data = pd.read_csv(
    "/N/project/waveform_mortality/shared/datasets/clinical_datasets/mimic-iii-clinical-database-1.4/ADMISSIONS.csv",
    usecols=["HADM_ID", "DISCHTIME", "HOSPITAL_EXPIRE_FLAG"],
)
admissions_data["DISCHTIME"] = pd.to_datetime(admissions_data["DISCHTIME"])
var1_data = pd.read_csv(
    "/N/project/waveform_mortality/georgelu/HeartRate.csv",
...
var2_data = pd.read_csv(
    "/N/project/waveform_mortality/georgelu/NIBPs.csv",
    usecols=["HADM_ID", "CHARTTIME", "VALUENUM"],
)
...
var1_admissions_combined["TIME_DIFFERENCE"] = (
    var1_admissions_combined["DISCHTIME"] - var1_admissions_combined["CHARTTIME"]
)
var2_admissions_combined["TIME_DIFFERENCE"] = (
    var2_admissions_combined["DISCHTIME"] - var2_admissions_combined["CHARTTIME"]
)

print(var2_admissions_combined.shape, var2_admissions_combined.shape)
assert False
...
while time <= max_time:
    ...
    var1_clean = var1_admissions_combined.drop(
        var1_admissions_combined[
            (var1_admissions_combined.TIME_DIFFERENCE < t1)
            | (var1_admissions_combined.TIME_DIFFERENCE > t2)
        ].index
    )
    var2_clean = var2_admissions_combined.drop(
        var2_admissions_combined[
            (var2_admissions_combined.TIME_DIFFERENCE < t1)
            | (var2_admissions_combined.TIME_DIFFERENCE > t2)
        ].index
    )
    var1_clean.drop(
        columns=["CHARTTIME", "DISCHTIME", "HOSPITAL_EXPIRE_FLAG", "TIME_DIFFERENCE"],
        inplace=True,
    )
    ...
    medians_data = pd.DataFrame(
        columns=["HADM_ID", var1, var2, "HOSPITAL_EXPIRE_FLAG"], dtype=np.float64
    )
    data_for_given_ID = pd.DataFrame(
        columns=["HADM_ID", var1, var2, "HOSPITAL_EXPIRE_FLAG"], dtype=np.float64
    )
    ...
    for ID in data_clean["HADM_ID"].drop_duplicates():
        ...
        is_expired = min(
            data_clean.loc[data_clean["HADM_ID"] == ID]["HOSPITAL_EXPIRE_FLAG"]
        )
    ...
    for index in indices:
        mortality_rate = pd.Series(
            [len(set(dead_points_indices).intersection(index)) / k]
        )
        mortality_rate_list = pd.concat([mortality_rate_list, mortality_rate])

    # SVM training
    X_train, X_test, y_train, y_test = train_test_split(
        medians_data[[var1, var2]],
        medians_data["HOSPITAL_EXPIRE_FLAG"],
        test_size=0.3,
        random_state=0,
    )
...
for index in indices:
    mortality_rate = pd.Series(
        [len(list(set(dead_points_indices).intersection(index))) / 3]
    )
    mortality_rate_list = pd.concat([mortality_rate_list, mortality_rate])
...
cntr = scatter.tricontourf(
    points[0], points[1], mortality_rate_list, levels=5, cmap="RdBu_r"
)
